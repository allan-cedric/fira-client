...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        if (visionClient->receive(packet) && packet.has_frame())
        {
            ...
            // Path planning
            int robots_n = robots_blue_n + robots_yellow_n - 1;
            vector<fira_message::sim_to_ref::Robot> robots_blue;

            for (int i = 0; i < robots_blue_n; i++)
            {
                fira_message::sim_to_ref::Robot robot_blue = detection.robots_blue(i);
                robot_blue.set_x((length + robot_blue.x()) * 100); //convertendo para centimetros
                robot_blue.set_y((width + robot_blue.y()) * 100);
                robot_blue.set_orientation(to180range(robot_blue.orientation()));
                robots_blue.push_back(robot_blue);
            }
